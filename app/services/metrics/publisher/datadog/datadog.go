// Package datadog provides support for publishing metrics to DataDog.
package datadog
...
func marshalDatadog(log *log.Logger, data map[string]any) ([]byte, error) {
	...
	// Populate the data into data structure
	var doc struct {
	...
	if err != nil {
		log.Println("datadog.publish: marshalling: ", err)
	}
