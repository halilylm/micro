...
func WithinTran(ctx context.Context, log *zap.SugaredLogger, db *sqlx.DB, fn func(tx *sqlx.Tx) error) error {
	...
	// We can defer the rollback since the code checks if the
...
// collection of data to be unmarshalled into a slice where field replacement is neccesary.
